...
> <p class="note"> Note
>
> If `pcluster create-cluster` fails with an error about **AWS::Route53::HostedZone** and **AccessDenied** (e.g., `route53:CreateHostedZone`), your AWS Organization likely blocks Route 53 via an SCP. Fix it by disabling ParallelCluster’s managed DNS:
> 1. Delete the failed cluster:
> ```shell
> (aws) $ pcluster delete-cluster -n MyCluster01
> ```
> 2. Edit ~/cluster-config.yaml and add the block exactly under Scheduling → SlurmSettings:
> ```yml
> Scheduling
>   SlurmSettings:
>   Dns:
...
> ```
> With managed DNS disabled, no Route 53 hosted zone will be created; the cluster will use default EC2 hostnames instead.
> 
