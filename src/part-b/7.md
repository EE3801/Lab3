# Part 6. Installing and testing software on AWS
...
(aws) $ pcluster delete-cluster --region ap-southeast-1 --cluster-name MyCluster01
```
...
```shell
(aws) $ pcluster ssh -i ~/MyKeyPair.pem --region ap-southeast-1 --cluster-name MyCluster01
```
...
```shell
(env1) [ec2-user@ip-10-0-5-43 data] $ curl -L -o eyelink-display-software_1.11_x64_debs.tar.gz https://www.dropbox.com/s/l1z4i4vi00f5ooc/eyelink-display-software_1.11_x64_debs.tar.gz?dl=0
```
...
```shell
(env1) [ec2-user@ip-10-0-5-43 data] $ tar -xvzf eyelink-display-software_1.11_x64_debs.tar.gz
```
...
```shell
(env1) [ec2-user@ip-10-0-5-43 x64] $ cp -p /lib/libedfapi.so /data/miniconda3/envs/env1/lib/
```
...
(env1) [ec2-user@ip-10-0-5-43 src] $ git clone https://github.com/grero/DataProcessingTools

(env1) [ec2-user@ip-10-0-5-43 src] $ git clone https://github.com/nwilming/pyedfread
```
